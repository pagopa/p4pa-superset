...
class RoleAndPermissionManager:
    ...
    def manage_user_roles_and_permissions(self, sm: SupersetSecurityManager, user, pu_user_info: PUUserInfo, http_headers):
        user.roles = [role for role in user.roles if role.name not in pu_superset_roles]
    ...
    def __assign_datasource_permissions_to_user(self, sm: SupersetSecurityManager, pu_user_info: PUUserInfo, user):
        if self.__is_operator(pu_user_info):
            ...
            self.__assign_datasource_permissions_to_role(sm, OPERATOR_DATASOURCE_ACCESS_PERMISSION_LIST, role)
            ...
            self.__assign_datasource_permissions_to_role(sm, ORGANIZATION_ADMIN_DATASOURCE_ACCESS_PERMISSION_LIST, role)
        elif self.__is_broker_admin(pu_user_info):
            role = self.__create_and_assign_role_to_user(sm, "broker_admin_access_role", user)
            self.__assign_datasource_permissions_to_role(sm, BROKER_ADMIN_DATASOURCE_ACCESS_PERMISSION_LIST, role)
        else:
            role = self.__create_and_assign_role_to_user(sm, "default_access_role", user)
            self.__assign_datasource_permissions_to_role(sm, DEFAULT_DATASOURCE_ACCESS_PERMISSION_LIST, role)
    ...
    def __assign_datasource_permissions_to_role(self, sm: SupersetSecurityManager, datasource_name_list_string, role):
        datasource_name_list = datasource_name_list_string.split(",")
        view_menu_name_list = []
